...
void fitM2_PID(const Char_t *inFileNameWithHisto = "iTest.root", const Char_t *inFileNameWithFitFun = "iTest1.root", const Char_t *outFileName = "oTest2.root", const Int_t energy = 39, const Int_t step = 30){
  	...
  	if(step==2){
		...
		FirstFitTree2x2DGaus(inFileWithHisto, inFileWithFitFun, outFile, 4);
		outFile->Close();
		inFileWithHisto->Close();
...
void FitTree2x2DGaus(TFile *inFile, TFile *outFile, TH2D *histo, Int_t pt, Int_t charge){
	...
	std::cout<<"GGGOOOOOOOOOOOOOOOOOD\n\n\n\n";
