...
void nSigmaM2_PID(const Char_t *inFileNameWithHisto = "iTest.root", const Char_t *outFileName = "oTest.root", const Int_t energy = 39, const Int_t step = 30){
  	...
  	if(step==1){
		...
		inFileWithHisto = new TFile(Form("%s",inFileNameWithHisto), "READ");
  		...
  		outFile = new TFile(Form("%s",outFileName), "UPDATE");
